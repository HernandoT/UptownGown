import "./PopularCollections.css";

const PopularCollections = () => {
  return (
    <div className="popularCollections">
      <div className="popularCollectionsTitle">
        <div className="strip"></div>
        <p>Popular Collections</p>
      </div>
      <div className="popularCollectionsContent">
        <div className="popularCollections1">1</div>
        <div className="popularCollections2">2</div>
        <div className="popularCollections3">3</div>
        <div className="popularCollections4">4</div>
      </div>
    </div>
  );
};

export default PopularCollections;
